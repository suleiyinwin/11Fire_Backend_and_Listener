...
import { emitSystemStartup } from "./utils/eventSystem.js";
...
mongoose
  .connect(process.env.MONGODB_URI || "mongodb://localhost:27017/11fire")
...
server.on("upgrade", (req, socket, head) => {
  if (req.url === "/ws/simulator") {
    wssSimulator.handleUpgrade(req, socket, head, (ws) => {
      wssSimulator.emit("connection", ws, req);
    });
  }
});
...
// Set simulator WebSocket server
simulatorService.setWebSocketServer(wssSimulator);
...
server.listen(port, () => {
  ...
  console.log(`  - Provider: ws://localhost:${port}/ws/provider`);
  console.log(`  - Bootstrap: ws://localhost:${port}/ws/bootstrap`);
  console.log(`  - Simulator: ws://localhost:${port}/ws/simulator`);

  try {
    simulatorService.initialize();
    eventHandlerService.initialize();
    console.log("Simulator services initialized successfully");

    // Emit system startup event
    emitSystemStartup();
  } catch (error) {
    console.error("Failed to initialize simulator services:", error);
  }
});
